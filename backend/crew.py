...
class QuestionAndSnippetGeneratorCrew:
    ...
    def snippet_identification_task(self) -> Task:
        ...
        """
        try:
            return Task(config=self.tasks_config['snippet_identification_task'])
        except Exception as e:
    ...
    def generate_questions_task(self) -> Task:
        ...
        """
        try:
            return Task(config=self.tasks_config['generate_questions_task'])
        except Exception as e:
...
# questions = crew_instance.crew().kickoff(inputs={"resume": latex, "today": datetime.now().strftime("%Y-%m-%d")})
# print(questions)
